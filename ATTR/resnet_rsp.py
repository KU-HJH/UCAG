...
class BasicBlock(nn.Module):
    ...
    def RSP_relprop(self, R):
        out = self.relu2.RSP_relprop(R)
        out, x = self.add.RSP_relprop(out)
        x_tmp = x
        # x_tmp = F.avg_pool2d(x, 2, stride=2, padding=1)
        # x_tmp = F.interpolate(x_tmp, x.size()[-1], mode='nearest')
        # x_tmp /= x_tmp.sum()
        # x_tmp *= x.sum()
        if self.downsample is not None:
            x_tmp = self.downsample.RSP_relprop(x_tmp)

        out = self.bn2.RSP_relprop(out.unsqueeze(0))
        out = self.conv2.RSP_relprop(out)

        out = self.relu1.RSP_relprop(out)
        out = self.bn1.RSP_relprop(out)
        x1 = self.conv1.RSP_relprop(out)

        return self.clone.RSP_relprop([x1, x_tmp])
        # return x1
class Bottleneck(nn.Module):
    ...
    def RSP_relprop(self, R):
        ...
        out = self.bn3.RSP_relprop(out)
        out = self.conv3.RSP_relprop(out)

        out = self.relu2.RSP_relprop(out)
        out = self.bn2.RSP_relprop(out)
        out = self.conv2.RSP_relprop(out)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False):
        ...
        self.avgpool = AvgPool2d((7, 7), stride=1)
    ...
    def relprop(self, R, alpha):
        ...
        return R
    def RSP_relprop(self, R):
        R = self.fc.RSP_relprop(R)
        R = R.reshape_as(self.avgpool.Y)
        R = self.avgpool.RSP_relprop(R)

        R = self.layer4.RSP_relprop(R)
        R = self.layer3.RSP_relprop(R)
        R = self.layer2.RSP_relprop(R)
        R = self.layer1.RSP_relprop(R)

        R = self.maxpool.RSP_relprop(R)
        R = self.relu.RSP_relprop(R)
        R = self.bn1.RSP_relprop(R)
        R = self.conv1.RSP_relprop(R)
