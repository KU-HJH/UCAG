...
def forward_hook(self, input, output):
    ...
    self.Y = output
    # self.register_backward_hook(self.save_gradient)
...
class RelProp(nn.Module):
    ...
    def __init__(self):
        ...
        # self.register_backward_hook(save_gradient)
    ...
    def relprop(self, R, alpha):
        return R
    def e_relprop(self, R, alpha):
        return R
    def RSP_relprop(self, R_p):
        return R_p
    ...
    def RSP_relprop(self, R_p):
        def backward(R_p):
            if torch.is_tensor(self.X) == True:
                self.X = self.X + 1e-9
                Z = self.forward(self.X)
                R_pos = torch.clamp(R_p, min=0)
                R_neg = torch.clamp(R_p, max=0)
                Sp = safe_divide(R_pos, Z)
                Cp = self.gradprop(Z, self.X, Sp)[0]
                Sn = safe_divide(R_neg, Z)
                Cn = self.gradprop(Z, self.X, Sn)[0]
            else:
                # self.X[0] = self.X[0] + 1e-9
                # self.X[1] = self.X[1] + 1e-9
                self.X[0] = torch.clamp(self.X[0], min=0) + 1e-9
                self.X[1] = torch.clamp(self.X[1], min=0) + 1e-9
                Z = self.forward(self.X)
                Sp = safe_divide(R_p, Z)
                Cp = self.gradprop(Z, self.X[0], Sp)[0]
                Sn = safe_divide(R_p, Z)
                Cn = self.gradprop(Z, self.X[1], Sn)[0]
                # div_val = R_p.unsqueeze(0) / 2
            if torch.is_tensor(self.X) == False:
                Rp = []
                Rp.append(self.X[0] * (Cp))
                Rp.append(self.X[1] * (Cn))
                # Rp.append(div_val)
                # Rp.append(div_val)
            else:
                Rp = self.X * (Cp + Cn)
...
class Conv2d(nn.Conv2d, RelProp):
    ...
    def RSP_relprop(self, R_p):
        def shift_rel(R, R_val):
            ...
            K = R - shift
            return K
        def pos_prop(R_pos, R_neg, Ga, Gb, x_pos):
            ...
            C = 2*(C1 - C2)
            # C = (C1 - C2)
            C = shift_rel(C, (R_pos - R_neg).sum(dim=[1,2,3], keepdim=True))

            return C

        def f(R, w1, w2, x1, x2):
            rel = R.clone()
            rel_sum = rel.sum([0,1], keepdim=True)
            pos_val = rel_sum.gt(0).type(rel.type()) * torch.ones_like(R) * R
            neg_val = rel_sum.le(0).type(rel.type()) * torch.ones_like(R) * R
            rel = pos_val + neg_val
            R = rel
            R_pos = torch.clamp(R, min=0)
            R_neg = -torch.clamp(R, max=0)
            R_ge = R_pos.ne(0).type(R.type())
            R_le = R_neg.ne(0).type(R.type())
            w = w1+w2
            Zp1 = F.conv2d(x1, w, bias=None, stride=self.stride, padding=self.padding) * R_ge
            Zp2 = F.conv2d(x1, w, bias=None, stride=self.stride, padding=self.padding) * R_le
            pos_grad = torch.autograd.grad(Zp1, w, R_pos, retain_graph=True)[0]
            neg_grad = torch.autograd.grad(Zp2, w, R_neg, retain_graph=True)[0]
            Ga = F.conv2d(x1, pos_grad, bias=None, stride=self.stride, padding=self.padding) * R_ge
            Gb = F.conv2d(x1, neg_grad, bias=None, stride=self.stride, padding=self.padding) * R_le
            if w1.shape[2] == 1:
                S = safe_divide(R_pos, Ga)
                C = x1 * self.gradprop(Ga, x1, S)[0]
                S2 = safe_divide(R_neg, Gb)
                C2 = x1 * self.gradprop(Gb, x1, S2)[0]
                C = C - C2
                return C
            else:
                C1 = pos_prop(R_pos, R_neg, Ga, Gb, x1)
            return C1
