...
class EnergyPointingGame:
    ...
    def __init__(self, num_classes, tolerance=15):
        ...
        self.energies = torch.zeros((num_classes,), dtype=torch.float64)
        self.energies_idx = torch.zeros((num_classes,), dtype=torch.float64)

    def evaluate(self, mask, map):
        mask_bbox = map * mask
        
        energy_bbox = mask_bbox.sum()
        energy_whole = map.sum()
        
        proportion = energy_bbox / energy_whole
        
        proportion = torch.nan_to_num(proportion, nan=0.0)
        # if proportion.isnan().any():
        #     return -1
        # print(proportion)
        return proportion.item()

    def aggregate(self, e, class_id):
        """Add pointing result from one example."""
        if e >= 0:
            self.energies[class_id] += e
            self.energies_idx[class_id] += 1

    def reset(self):
        """Reset hits and misses."""
        self.energies = torch.zeros_like(self.energies)
        self.energies_idx = torch.zeros_like(self.energies_idx)
    ...
    def class_accuracies(self):
        ...
        energies_idx = self.energies_idx
        energies_idx[energies_idx == 0] = 1
        return self.energies / energies_idx
    ...
    def __str__(self):
        energies_idx = self.energies_idx
        energies_idx[energies_idx == 0] = 1
        class_accuracies = self.energies / energies_idx
        # class_accuracies = torch.nan_to_num(class_accuracies, nan=0.0)
    ...
    def __init__(self, dataset, tolerance=15, difficult=False):
        ...
        if difficult:
            def load_flags(name):
                ...
                with open(os.path.join('benchmark', name), 'r') as file:
...
class EnergyPointingGameBenchmark(EnergyPointingGame):
    """Pointing game benchmark on standard datasets.
    ...
    * (:class:`torchvision.CocoDetection`) COCO *val2014* subset.
    ...
    def __init__(self, dataset, tolerance=15, difficult=False):
        ...
        super(EnergyPointingGameBenchmark, self).__init__(
            num_classes=num_classes, tolerance=tolerance)
        self.dataset = dataset
        self.difficult = difficult
    ...
    def evaluate(self, label, class_id, map):
        """Evaluate an label-class-point triplet.
        ...
        assert mask is not None
        return super(EnergyPointingGameBenchmark, self).evaluate(mask.to(map.device), map.squeeze(0).squeeze(0))