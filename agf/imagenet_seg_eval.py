...
from modules.resnet_fg import resnet50 as resnet50_fg
from modules.vgg import vgg19 as vgg19_agf
from modules.resnet import resnet50 as renset50_agf
from modules.vgg_rap import vgg19 as vgg19_rap
from modules.resnet_rap import resnet50 as resnet50_rap
...
if args.arc == 'vgg':
    if 'agf' in args.method:
        if args.name == 'resnet50':
            print('agf + resnet50')
            model = renset50_agf(pretrained=True).cuda()
        ...
        if args.method == 'rap':
            args.batch_size = 1
        if args.name == 'resnet50':
            print('run original resnet50 rap')
            model = resnet50_rap(pretrained=True).to(device)
        elif args.name == 'vgg19':
            print('run original vgg rap')
    ...
    elif args.method == 'integradgradients':
        args.batch_size = 1
        model = vgg19(pretrained=True).to(device)
        ...
        args.BP = get_cam(args.cam, model, args.target_layer, args)
    else:
...
def wrapper(generator, images, args, target, kwargs=None):
    args.lrp_based = True
    args.kwargs = kwargs
    args.evaluator = generator
    return multi_scale_fusion(generator, images, target=target, args=args)
    
# def rap_wrapper(generator, images, args, target, kwargs=None):
#     args.evaluator = generator
#     return multi_scale_fusion(generator, images, target=target, args=args)
...
def eval_batch(image, labels, evaluator):
    ...
    if 'agf' in args.method:
        ...
        if args.ours:
            Res = wrapper(evaluator, images, args, target=predictions.argmax(1), kwargs=kwargs)
        else:
        ...
        pred_class = predictions.data.max(1, keepdim=True)[1].squeeze(1).item()
        if args.ours:
            cam = multi_scale_fusion(args.BP, image, pred_class, args=args, image_size=(224, 224))
        else:
            if args.image_size != 224:
                cam = args.BP(F.interpolate(image, size=(args.image_size, args.image_size), mode='bicubic', align_corners=False),
                        pred_class, image_size=(224, 224))
            else:
                cam = args.BP(image,
                        pred_class, image_size=(224, 224))
        # if args.cam in ['integrad', 'fullgrad', 'smoothgrad', 'lrp']:
        #     Res = cam.abs()
        #     Res = Res - Res.mean()
        # else:
        Res = cam
    ...
    if args.cam in ['rollout', 'full_lrp', 'transformer_attribution', 'lrp_last_layer', 'attn_last_layer', 'attn_gradcam']:
