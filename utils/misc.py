...
def convert_to_gray(x, percentile=99):
    ...
    """
    x_2d = torch.abs(x).sum(dim=1, keepdim=True)
    v_max = torch.stack([torch.tensor([np.percentile(x, percentile)]) for x in range(x_2d.size(0))], dim=0).view(x_2d.size(0), 1, 1, 1)
    # v_max = np.percentile(x_2d, percentile)
    # v_min = torch.min(x_2d)
    v_min = torch.min(x_2d, dim=-1)[0]
    v_min = v_min.min(dim=-1)[0].view(x_2d.size(0), 1, 1, 1)
    
    x_2d = torch.clamp((x_2d - v_min) / (v_max - v_min), 0, 1)
    return x_2d
